...
Write your answer to `/root/policy-checker.txt` using exactly three lines:
...
- line 3: the required node**Affinity** rule's **key/value** pair (from `nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution`), formatted as `key=value`

When `/root/policy-checker.txt` has all three lines correct, click **Check**.
